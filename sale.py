...
class sale_stockout(osv.osv):
    _name = "sale.stockout"
    _description = "Modelo con los productos que tuvieron" \
        " stockout durante la confirmacion del pedido"
    _columns = {
        ...
        for sale in sale_obj.browse(cr, uid, sale_ids, context=context):
            for line in sale.order_line:
                if line.product_id.qty_available < line.product_uom_qty:
                    stock_out_id = self.search(cr, uid, [
                        ('sale_id', '=', sale.id),
                    ...
                    if not stock_out_id:
                        ...
                        self.create(cr, uid, vals_stock_out)
...
class sale_order(osv.osv):
    ...
    def action_quotation_send(self, cr, uid, ids, context=None):
        ...
        if obj and not obj.discount_ok:
                raise osv.except_osv(('Alerta!'),
                                     ("El descuento necesita ser aprobado"))
        return super(sale_order, self).action_quotation_send(cr,
                                                             uid,
                                                             ids,
                                                             context=context)
    ...
    def print_quotation(self, cr, uid, ids, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, ids,
                                              'sba_sales.report_saleorder_sba',
                                              context=context)
    ...
    def send_survey(self, cr, uid, survey_id, partner_id, context=None):
        ...
        def create_response_and_send_mail(token, partner_id, email,
                                          template_id):
            """
            Create one mail by recipients and replace __URL__
            by link with identification token
            """
            url = survey.public_url
            template_id = self.pool.get('email.template').search(
                cr, uid, [('model', '=', 'survey.survey')])
            ...
            values = {
                ...
                partner_id and [(4, partner_id)] or None,
        ...
        def create_token(partner_id, email):
            if context.get("survey_resent_token"):
                response_ids = survey_response_obj.search(
                    cr, uid, [('survey_id', '=', survey_id),
                              ('state', 'in', ['new', 'skip']),
                              '|',
                              ('partner_id', '=', partner_id),
                              ('email', '=', email)], context=context)
                if response_ids:
                    return survey_response_obj.read(
                        cr, uid, response_ids,
                        ['token'], context=context)[0]['token']
        ...
        create_response_and_send_mail(
            token, partner_id, partner.email, template_id)
    ...
    def action_button_confirm(self, cr, uid, ids, context=None):
        ...
        if obj.survey_id:
            self.send_survey(
                cr, uid, obj.survey_id.id, obj.partner_id.id, context)
        return r
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if ids and 'discount_ok' not in vals.keys():
            obj = self.browse(cr, uid, ids[0], context=context)
            if vals.get('state', obj.state) in ['draft']:
                vals['discount_ok'] = not float(
                    vals.get('add_disc', obj.add_disc)) > 0

        return super(sale_order, self).write(
            cr, uid, ids, vals, context=context)

    def _sba_check_discount(self, cr, uid, ids, context=None):
            obj = self.browse(cr, uid, ids[0], context=context)
            if obj.state in ('manual', 'sent'):
                    if obj.discount_ok:
                            return True
    ...
    def _sba_check_limit(self, cr, uid, ids, context=None):
        obj = self.browse(cr, uid, ids[0], context=context)
        return (
            obj.state in ('manual', 'sent') and
            obj.partner_id.credit + obj.amount_total
            <= obj.partner_id.credit_limit
        ) or obj.state not in ('manual', 'sent')

    _constraints = [
        (_sba_check_discount,
         '\n\nUd acaba de otorgar un descuento superior al descuento que'
         ' se le permite otorgar.\n'
         'Por favor, pida a su superior que autorice el pedido',
         ['add_disc', 'state']),
        (_sba_check_limit,
         '\n\nEl cliente no tiene crédito para validar esta compra',
         ['amount_total', 'state']),
    ]
    ...
    def approve_discount(self, cr, uid, ids, context=None):
        ...
        for so in self.browse(cr, uid, ids, context=context):
            ...
            discount = 0 if not team else team.discount
            ...
            if not discount_ok:
                ...
                if boss == user:
                    boss = so.section_id.parent_id.user_id if \
                        so.section_id.parent_id else False

                if not boss:
                    message_id = self.message_post(
                        cr, uid, [so.id],
                        subject=_('Request action'),
                        body=_('The sale order has not a valid sale team. '
                               'Assign one.'))
                    self.pool.get('mail.message').set_message_read(
                        cr, user.id, [message_id], False)
                else:
                    self.message_subscribe_users(
                        cr, uid, [so.id], user_ids=[boss.id])
                    message_id = self.message_post(
                        cr, uid, [so.id],
                        subject=_('Request action'),
                        body=_('The user <b>%s</b> tried to approve this sale'
                               ' order, but the user does not have the'
                               ' authorization to approve such discount.\n'
                               'An approval request for this sale order has'
                               ' been created for user <b>%s</b>.')
                        % (user.name, boss.name), to_read=True)
                    self.pool.get('mail.message').set_message_read(
                        cr, boss.id, [message_id], False)
            else:
                self.message_post(
                    cr, uid, [so.id],
                    body=_('The user <b>%s</b> approve this sale'
                           ' order with %4.2f%% discount.')
                    % (user.name, so.add_disc))
...
class sale_order_line(osv.osv):
    ...
    def onchange_discount(self, cr, uid, ids, discount, context=None):
        ...
        if obj.discount > discount:
            ...
            warning = {
                ...
                'message': _('Si cambia la lista de precios, no se '
                             'modificaran los precios a los productos '
                             'ya cargados.'),
                }
            return {'warning': warning, 'value': res}
        return {'value': res}

    def _fnct_listprice_unit(self, cr, uid, ids, field_name, args,
                             context=None):
        if context is None:
            context = {}
        res = {}
    ...
    _columns = {
        'original_price': fields.related(
            'product_id', 'list_price', string='Original Price', readonly=True),
        'original_discount': fields.float(
            'Original Discount', readonly=True),
        'list_price_perunit': fields.function(
            _fnct_listprice_unit, string='Precio Publico'),
        }
